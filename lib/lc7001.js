		...
		while (this.responseQueue.length > 0) {
			...
			if (response.ID == 0 || response.ID == undefined) {
				if (response.ArtikEvent !== undefined) {
					// Reduce log size and stop printing this message.
					// this.log('ArtikEvent ignored.');
				} else if (response.MAC !== undefined) {
					this.MAC = response.MAC;
				} else if (/^[0-9]{6}Debug/.test(response.Context)) {
					this.log('LC7001 Debug Message: ' + response.Context);
					...
					switch(response.Service) {
						...
						case 'BroadcastMemory':
							...
							break;
						case 'EliotErrors':
							//There is nothing to do with these messages.
							//this.log('EliotErrors ignored.');
							break;
						case 'ping':
						...
						case 'SystemInfo':
							...
							if (response.UpdateState !== undefined) {
								if (response.UpdateState.Status == 'Ready') {
									this.log('LC7001 ready to update to firmware version ' + response.UpdateState.UpdateVersion + ': ' + response.UpdateState.ReleaseNotes)
								}
							}
							break;
						...
						case 'ZonePropertiesChanged':
							...
							if (this.accessories[response.ZID].PropertyList === undefined) {
						...
						default:
							...
							} else if (/^\*\*\*\*\*\* NTP one shot did not work/.test(response.Service)) {
								this.log('NTP failed on LC7001.');
							} else if (/^\*\*\*\*\*\* NTP ALT TIME WORKED/.test(response.Service)) {
								this.log('Alternative NTP successful.')
								this.lastNTPTime = response.PropertyList.CurrentTime;
							} else {
				...
				switch(response.Service) {
					case 'ListZones':
						...
						if (response.ZoneList.length == 0) {
							this.log('LC7001 reports having no zones.');
						} else {
					...
					case 'ReportZoneProperties':
						...
						if (!this.isInitialized) {
							...
							if (this.isInitialized) {
								this.log('Finished polling LC7001 for accessories.');
...
lc7001.interface.on('close', function() {
	this.log('Connection to LC7001 closed.');
	this.log('Reconnecting to LC7001.');
	this.interface.connect(this.tcpOptions,function() {
	...
	splitdata.forEach(function(value,index,array) {
		if (value.length > 0) {
			try {
				...
				//Hack to deal with undelimited JSON.
				//Remove by making Else statement only Catch code.
				if (value.indexOf('}{') >= 0) {
					console.log('Error parsing JSON. Possible undelimited JSON detected! Replacing all }{s with delimited version.');
					var splitdata2 = value.replace(/\}\{/g, ('}' + this.delimiter + '{')).split(this.delimiter);
					splitdata2.forEach(function(value,index,array) {
						try {
							this.responseQueue.push(JSON.parse(value));
						} catch(err) {
